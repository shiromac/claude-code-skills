...
`.claude/` ディレクトリをプロジェクトルートにコピーしてください。

```bash
# リポジトリをクローン
git clone https://github.com/<your-username>/claude-code-review-skills.git

# プロジェクトにコピー
cp -r claude-code-review-skills/.claude/ /path/to/your-project/.claude/
```

既に `.claude/` ディレクトリがある場合は、中身をマージしてください:

```bash
cp -r claude-code-review-skills/.claude/skills/* /path/to/your-project/.claude/skills/
cp -r claude-code-review-skills/.claude/commands/* /path/to/your-project/.claude/commands/
```
