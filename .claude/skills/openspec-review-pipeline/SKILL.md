...
specs/ を作成（または既に存在）した場合、`test/<Project>.Tests/Integration/<feature>/` 配下に BDD 結合テストが存在するか確認する。**存在しなければ specs レビューに入る前に作成する**:

- 作成手順はプロジェクト標準の BDD test authoring workflow に従う（テスト設計・配置先・Red/Green 判定の詳細はそちらを参照）
- 完了条件: 全テストが対象機能未実装のため **Red** になること（既に Green なら Assert 弱化または機能が既に存在するため、プロジェクト標準の BDD test authoring workflow に従って対処）
...
実装がコード実行時の挙動、UI、プロトコル、永続化、課金/accounting、recovery、scenario progression、LLM/headless/native UI 経路に影響する場合、`tasks.md` に runtime verification task を作る。documentation-only / test-only / config-only の change だけは対象外としてよいが、その理由を `tasks.md` に明記する。
...
- プロジェクトの実装原則に含まれる Runtime Verification Procedure に基づく pre-flight pass condition
- バグ再現に必要な実シナリオ、操作経路、headless runtime / native UI app / UI automation のどれで確認するか
...
- **steering compliance（全 downstream docs 必須）**: プロジェクトの agent 指示と steering docs の必須ルールを要求ソースとして扱う。特に UI/protocol 変更では UI Projection vs Authoritative State、state ownership note、multi-host shared core、runtime verification procedure を確認する。違反・欠落は CRITICAL（label: `STEERING_CONFORMANCE_GAP`）
- **specs レビュー: BDD テスト同時レビュー（必須）** — specs/*.md と `test/<Project>.Tests/Integration/<feature>/*.cs` を 1 セットでレビュアーに渡す。観点:
  ...
  - **モック境界の妥当性** — プロジェクト標準の BDD / test-design guidance に従い、モック境界が production path を隠していないか確認する。LLM・外部 I/O 以外をモックしている、プロダクションパスをバイパスしているテストは CRITICAL
...
Apply the project's agent instructions and steering docs as mandatory requirements. For UI/protocol work, explicitly check UI Projection vs Authoritative State and the state-ownership note. For runtime behavior changes, explicitly check runtime verification tasks and evidence gates.
