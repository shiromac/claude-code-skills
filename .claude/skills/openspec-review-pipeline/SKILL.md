...
- **steering compliance（全 downstream docs 必須）**: `CLAUDE.md` と `docs/steering/*` の必須ルールを要求ソースとして扱う。特に UI/protocol 変更では UI Projection vs Authoritative State、state ownership note、multi-host shared core、runtime verification procedure を確認する。違反・欠落は CRITICAL（label: `STEERING_CONFORMANCE_GAP`）
