...
レビュアーの spawn 時に `model` パラメータでモデルを指定する。全レビュアーに最高性能モデルを使う必要はない。
...
| concurrency | sonnet | 並行性の分析。sonnet で十分 |
| performance | sonnet | パフォーマンス分析。sonnet で十分 |
| test-quality | sonnet | テスト品質チェック。sonnet で十分 |
| error-handling | sonnet | エラーハンドリング分析。sonnet で十分 |
| readability | sonnet | 可読性チェック。sonnet で十分 |
| ux | sonnet | UX 分析。sonnet で十分 |

**リーダー（親エージェント）は opus を維持する** — 妥当性検証（Step 5-2）は複数レビュアーの報告を統合し、誤検出を判定する高度な推論が必要なため。
